...
--[[
	不好用
	local layer = RubCardFunc({
							pList = "res/poker.plist", 
							szBack = "poker_0_0.png", 
 							szFont = "poker_1_3.png", 
 							scale = 1.0,
 							mode = true,
 							})
 	self:addChild(layer)
...
local function getTextureAndRange(szImage)
	...
	-- local bigWide = tex:getPixelsWide() --plist
	...
	if mode == true then--plist版本
		...
		print("-------------------------------------------------------------------------")
		print("ID = ", id)
		print("TABLE = ", ll, rr, tt, bb)
		print("宽高 = ", rect.width, rect.height)
		print("-------------------------------------------------------------------------")
		return id, {ll, rr, tt, bb}, {rect.width, rect.height}
	else
		local sprite = cc.Sprite:create(szImage)
		sprite:setPosition(480, 320)
		print("查看文件名称:  ",szImage,"\n"," 文件创建成功了吗",sprite)
		local bigWide = sprite:getContentSize().width 
		local bigHigh = sprite:getContentSize().height
		local rect = cc.Rect(-bigWide / 2, -bigHigh / 2, bigWide, bigHigh) 
		local ll, rr, tt, bb = rect.x/bigWide, (rect.x + rect.width)/bigWide, rect.y/bigHigh, (rect.y + rect.height)/bigHigh
		local id = 0
		print("-------------------------------------------------------------------------")
		print("ID = ", id)
...
local function createRubCardEffectLayer(params)
	pList = params.pList
	szBack = params.szBack
	szFont = params.szFont
	...
	layer:setContentSize(cc.size(20, 20))
	...
	local function touchMove(touch, event)
		...
		layer.ratioVal = cc.clampf(layer.ratioVal + dy/100, 0.0, 1.2) --弯曲程度最大程度默认0.9
		posNow = location
		return true
	end
