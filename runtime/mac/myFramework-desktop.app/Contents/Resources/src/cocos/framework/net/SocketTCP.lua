...
function SocketTCP:close( ... )
	...
	self:dispatchEvent({eventName=SocketTCP.EVENT_CLOSE})
end
...
function SocketTCP:_disconnect()
	...
	self:dispatchEvent({eventName=SocketTCP.EVENT_CLOSED})
end
...
function SocketTCP:_onConnected()
	...
	self:dispatchEvent({eventName=SocketTCP.EVENT_CONNECTED})
	if self.connectTimeTickScheduler then scheduler.unscheduleGlobal(self.connectTimeTickScheduler) end
	...
	local __tick = function()
		while true do
			...
			self:dispatchEvent({eventName=SocketTCP.EVENT_DATA, data=(__partial or __body), partial=__partial, body=__body})
		end
	end
...
function SocketTCP:_connectFailure(status)
	...
	self:dispatchEvent({eventName=SocketTCP.EVENT_CONNECT_FAILURE})
	self:_reconnect();
end
...
function SocketTCP:dispatchEvent( params )
	print("SocketTCP:dispatchEvent", params.eventName)
	local eventDispatcher = cc.Director:getInstance():getEventDispatcher()

	local event = cc.EventCustom:new(params.eventName)
